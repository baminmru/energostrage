...
Public Class frmDayProfilAn
    ...
    Private Sub tv_AfterSelect(sender As Object, e As SelectEventArgs) Handles tv.AfterSelect
        ...
        If n.Key.ToString().StartsWith("enodes:") Then
            ...
            dt = tvmain.QuerySelect("select p_date, sum(nvl(code_01,0)) as A_PLUS, sum(nvl(code_02,0)) as A_MINUS,sum(nvl(code_03,0))  as R_PLUS,sum(nvl(code_04,0)) as R_MINUS, sum(nvl(code_01,0)*0.75) as MINUS_25, sum(nvl(code_01,0)*1.05) as PLUS_5 from edata_agg join echanel on edata_agg.chanel_id=echanel.chanel_id and echanel.node_id=" + id.ToString + w + " group by p_date order by p_date")
            ...
            '    dc2 = New DataColumn("Minus_25", dt.Columns.Item("A_PLUS").DataType)
            '    dt2.Columns.Add(dc2)

            '    dc2 = New DataColumn("Plus_5", dt.Columns.Item("A_PLUS").DataType)
            '    dt2.Columns.Add(dc2)
