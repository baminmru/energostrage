...
Module Module1
    ...
    Public Sub Agregate()
        If MsgBox("Провести агрегацию данных для графиков?", MsgBoxStyle.YesNo + MsgBoxStyle.Question, "Агрегация данных") = DialogResult.Yes Then
            ...
            Dim dt As DataTable
            dt = tvmain.QuerySelect("select node_id from enodes where hidden=0")
            Dim nclr As Color
            Dim i As Integer
    ...
    Public NodeFilter As String = ""

    Public Sub LoadNodes(p As UltraTreeNode, sender_id As Integer)
        Dim dt As DataTable
        Dim q As String = ""
        If NodeFilter <> "" Then
            q = q + " and ( mpoint_name || ';' || mpoint_code || '-' || nvl(dogovor,'') || ';' || nvl(mpoint_serial,'')  || ';' ) like '%" + NodeFilter + "%'"
        End If


        dt = tvmain.QuerySelect("select * from enodes where hidden=0 and sender_id=" + sender_id.ToString() + q + " order by mpoint_name")
        Dim n As UltraTreeNode
        Dim i As Integer
        For i = 0 To dt.Rows.Count - 1
            Try
                n = New UltraTreeNode("enodes:" + dt.Rows(i)("node_id").ToString, dt.Rows(i)("mpoint_name") + " (" + dt.Rows(i)("mpoint_code") + "-" + dt.Rows(i)("dogovor") + " SN:" + dt.Rows(i)("mpoint_serial").ToString() & ")")
