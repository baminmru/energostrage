...
-- Дата скрипта: 30.05.2017 13:56:28
...
--
-- Описание для таблицы ECHANEL
--
CREATE TABLE ECHANEL (
  CHANEL_ID    NUMBER(*, 0),
  NODE_ID      NUMBER(*, 0) NOT NULL,
  MCHANEL_CODE VARCHAR2(6 BYTE),
  MCHANEL_DESC VARCHAR2(512 BYTE),
  CONSTRAINT PK_ECHANEL PRIMARY KEY (CHANEL_ID) USING INDEX TABLESPACE USERS STORAGE (INITIAL 64 K
                                                                                      NEXT 1 M
                                                                                      MAXEXTENTS UNLIMITED)
)
TABLESPACE USERS
...
COMMENT ON COLUMN ECHANEL.MCHANEL_CODE IS 'Код измерительного канала <measuringchannel code>';
COMMENT ON COLUMN ECHANEL.MCHANEL_DESC IS 'Текстовое описание измерительного канала <measuringchannel desc>';

--
-- Описание для таблицы EDATA
--
CREATE TABLE EDATA (
  DATA_ID   NUMBER(*, 0) NOT NULL,
  CHANEL_ID NUMBER(*, 0) NOT NULL,
  C_DATE    DATE,
  LIGHTSAVE VARCHAR2(1 BYTE),
  P_DATE    DATE,
  P_START   DATE,
  P_END     DATE,
  CODE_T    NUMBER(18, 6),
  ...
  CODE_03   NUMBER(18, 6),
  CODE_04   NUMBER(18, 6),
  CONSTRAINT PK_EDATA1 PRIMARY KEY (DATA_ID) USING INDEX TABLESPACE USERS STORAGE (INITIAL 64 K
                                                                                   NEXT 1 M
                                                                                   MAXEXTENTS UNLIMITED)
)
TABLESPACE USERS
...
COMMENT ON COLUMN EDATA.C_DATE IS 'Дата создания файла <timestamp>';
COMMENT ON COLUMN EDATA.CODE_01 IS 'Активный привем (A+)';
COMMENT ON COLUMN EDATA.CODE_02 IS 'Активная отдача (A-)';
COMMENT ON COLUMN EDATA.CODE_03 IS 'Реактивный прием (R+)';
COMMENT ON COLUMN EDATA.CODE_04 IS 'Реакитвная отдача (R-)';
COMMENT ON COLUMN EDATA.CODE_H IS 'Влажность (H)';
COMMENT ON COLUMN EDATA.CODE_L IS 'Освещенность (лк)';
COMMENT ON COLUMN EDATA.CODE_T IS 'Температура (''C)';
COMMENT ON COLUMN EDATA.LIGHTSAVE IS 'Время зимнее/летнее/признак перевода вр. 0/1/2 <daylightsavingtime>';
COMMENT ON COLUMN EDATA.P_DATE IS 'Дата операционного периода <day>';
COMMENT ON COLUMN EDATA.P_END IS 'Дата и время окончания периода  (формат - DTГГГГММДДччммсс)';
COMMENT ON COLUMN EDATA.P_START IS 'Дата и время начала периода (формат - DTГГГГММДДччммсс)';
...
CREATE TABLE EDATA_AGG (
  ...
  CODE_03   NUMBER(18, 6),
  CODE_04   NUMBER(18, 6),
  CONSTRAINT PK_EDATA_AGG PRIMARY KEY (CHANEL_ID, P_DATE) USING INDEX TABLESPACE USERS STORAGE (INITIAL 64 K
                                                                                                NEXT 1 M
                                                                                                MAXEXTENTS UNLIMITED)
)
TABLESPACE USERS
...
CREATE TABLE EDATA_WEEK (
  ...
  CODE_03   NUMBER(18, 6),
  CODE_04   NUMBER(18, 6),
  CONSTRAINT PK_EDATA_WEEK PRIMARY KEY (CHANEL_ID, YEAR, WEEK) USING INDEX TABLESPACE USERS STORAGE (INITIAL 64 K
                                                                                                     NEXT 1 M
                                                                                                     MAXEXTENTS UNLIMITED)
)
TABLESPACE USERS
...
CREATE TABLE EDATA_WEEKMULT (
  ...
  CODE_04   NUMBER(18, 6),
  AR        NUMBER(18, 6),
  CONSTRAINT PK_EDATA_WEEKMULT PRIMARY KEY (CHANEL_ID, WEEK) USING INDEX TABLESPACE USERS STORAGE (INITIAL 64 K
                                                                                                   NEXT 1 M
                                                                                                   MAXEXTENTS UNLIMITED)
)
TABLESPACE USERS
