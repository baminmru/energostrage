...
Public Class Matrix
    ...
    Public Sub LoadPriceMatrix(aPriceCategory As String, aMinPower As Double, aMaxPower As Double, aPowerLevel As String, Y As Integer, M As Integer, Tarif As Integer, aMatrixType As enumMatrixType, Optional ByVal aSUBTARRIF As String = "")
        Dim dt As DataTable
        Dim q As String
        If aSUBTARRIF = "" Then
            ...
            If aMaxPower <= 670 Then
                q = "select * from pr_info
                          WHERE powerlevel='" + aPowerLevel + "' and category='" + aPriceCategory + "'  AND MAXPOWER=670 
                          and SUBTARRIF is null  AND THEYEAR=" + Y.ToString() + " AND THEMONTH=" + M.ToString() + " and TARIFID=" + Tarif.ToString()
            Else
                q = "select * from pr_info
                              WHERE powerlevel='" + aPowerLevel + "' and category='" + aPriceCategory + "'  AND MINPOWER=" + aMinPower.ToString() + " AND MAXPOWER=" + aMaxPower.ToString() + "
    ...
    Public Sub LoadPeekMatrix(aPriceCategory As String, Y As Integer, M As Integer, TARIF As Integer, aMatrixType As enumMatrixType)
        Dim dt As DataTable
        Dim q As String
    ...
    Public Sub LoadData(ByVal NodeID As Integer, Y As Integer, M As Integer)
        ...
        'dt = tvmain.QuerySelect("select  to_char(p_DATE,'YYYY') Y,  to_char(p_DATE,'MM') M, to_char(p_DATE,'dd') D,  to_char(p_start,'HH24') p_hour, sum(nvl(code_01,0)) as AP ,
        '    sum (nvl(code_02,0)) as AM,
        '    sum(nvl(code_03,0)) as RP ,
        '    sum(nvl(code_04,0)) as RM ,
        '    node_id
        '     from v_EDATA
        '     WHERE node_id=" + NodeID.ToString() + "  AND to_char(p_DATE,'YYYY')=" + Y.ToString() + " AND to_char(p_DATE,'MM')=" + M.ToString() + "
        '        group by node_id,p_date, to_char(p_start,'HH24')
        '        ORDER BY p_date,to_char(p_start,'HH24')
        '")
    ...
    Public Sub AddData(ByVal NodeID As Integer, Y As Integer, M As Integer)
        Dim dt As DataTable
        dt = tvmain.QuerySelect("select  to_char(p_DATE,'YYYY') Y,  to_char(p_DATE,'MM') M, to_char(p_DATE,'dd') D,  to_char(p_end,'HH24') p_hour, sum(nvl(code_01,0)) as AP ,
            sum (nvl(code_02,0)) as AM,
            sum(nvl(code_03,0)) as RP ,
