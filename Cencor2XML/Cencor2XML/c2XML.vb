...
Namespace Cencor2XML
    Public Class c2XML
        ...
        Public Property XmlFile() As String
            Get
                If mXmlFile = "" Then
                    mXmlFile = System.IO.Path.GetDirectoryName(Me.GetType().Assembly.Location) + "\C2XML_Config.xml"
                End If
                XmlFile = mXmlFile
            End Get
        ...
        Private Sub ProcessData()
            Dim dt As DataTable
            dt = QuerySelect("select ID,[Название]  as Name, [Номер_счётчика] as CNumber ,[Коэффициент_трансформации] KT  , [Коэффициент_потерь] KP,[Мощность] PWR from  [ТУ] where [Удалена]=0")
            Dim i As Integer
            ...
            For i = 0 To dt.Rows.Count - 1
                ...
                node.Attributes.Append(xattr)
                xattr = xml.CreateAttribute("kt")
                xattr.Value = "" & dt.Rows(i)("KT")
                node.Attributes.Append(xattr)
                xattr = xml.CreateAttribute("kp")
                xattr.Value = "" & dt.Rows(i)("KP")
                node.Attributes.Append(xattr)
                xattr = xml.CreateAttribute("pwr")
                xattr.Value = "" & dt.Rows(i)("PWR")
                node.Attributes.Append(xattr)
            ...
            xml.Save(lPath + "c2xml_" + StartDate.ToString("yyyyMMdd") + ".xml")
        ...
        Private Sub SavePeriod(ByRef chan As XmlElement, ByVal cID As Integer)
            Dim dt As DataTable
            dt = QuerySelect("select [Показание] info,[ВремяТУ] itime from [Показания] where IDТУ=" + cID.ToString() + " and [ВремяТУ]>=" + MakeMSSQLDate(StartDate) + "  order by [ВремяТУ]")
            Dim i As Integer
