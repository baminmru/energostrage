Public Class frmTarif
    ...
    Private Sub frmTarif_FormClosed(sender As Object, e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        fdev = Nothing
    End Sub
    ...
    Private Sub refreshGrid()
        ...
        dt = TvMain.QuerySelect("select *  from tarif order by name")
        GV.DataSource = dt
        Dim i As Integer
    ...
    Private Sub cmdDel_Click(sender As System.Object, e As System.EventArgs) Handles cmdDel.Click
        If MsgBox("Удалить текущую запись ?", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            Try
                Dim s As String
                s = "delete from tarif where tarifid=" + id.ToString
                TvMain.QueryExec(s)
                refreshGrid()
            Catch ex As Exception
