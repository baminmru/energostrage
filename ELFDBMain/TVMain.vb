...
Public Class TVMain
    ...
    Private m_DBTableName As String = "electro"
    ...
    Public Sub ClearDBArchString(ByVal ArchType As Int32, ByVal ArchYear As Int32,
    ...
    Public Sub PrepareDataWork(ByVal DateFrom As Date, ByVal DateTo As Date)
        ...
        cmd.CommandText = "insert into datawork " & DBTableName & " where dcounter>=" +
        OracleDate(DateFrom) + " and dcounter<=" +
        OracleDate(DateTo) + ""
    ...
    Public Function CheckForArch(ByVal ArchType As Int32, ByVal ArchYear As Int32,
    ...
    Public Function GetDBDevicePlanListIP() As DataTable
        ...
        " join plancall on bdevices.id_bd=plancall.id_bd where  transport in (2,3) and npquery=1 and( nplock is null or nplock < sysdate) " &
        " and (plancall.dlock is null or plancall.dlock + plancall.MINREPEAT/24/60 < sysdate ) and plancall.ncall < plancall.nmaxcall " &
        " and ((plancall.chour=1 and nvl(plancall.DNEXTHOUR,sysdate-1)<=sysdate) or (plancall.ccurr=1 and nvl(plancall.dnextcurr,sysdate-1) <=sysdate) or (plancall.c24=1 and nvl(plancall.dnext24 ,sysdate-1)<=sysdate)  or (plancall.csum=1 and nvl(plancall.dnextsum ,sysdate-1)<=sysdate)) " + " "
    ...
    Public Function GetDBDevicePlanListModem() As DataTable
        ...
        cmd.CommandText = "select plancall.*,npip,nppassword,ipport,transport,sysdate ServerDate from bdevices" +
            " join ipaddr on bdevices.srv_ip_id=ipaddr.id_ip  and ipaddr.terminal='" + MyName + "' " +
            " join plancall on bdevices.id_bd=plancall.id_bd where  transport in (0,4) and npquery=1 and( nplock is null or nplock < sysdate) " &
            " and (plancall.dlock is null or plancall.dlock + plancall.MINREPEAT/24/60 < sysdate ) and plancall.ncall < plancall.nmaxcall " &
            " and ((plancall.chour=1 and nvl(plancall.DNEXTHOUR,sysdate-1)<=sysdate) or (plancall.ccurr=1 and nvl(plancall.dnextcurr,sysdate-1) <=sysdate) or (plancall.c24=1 and nvl(plancall.dnext24 ,sysdate-1)<=sysdate)  or (plancall.csum=1 and nvl(plancall.dnextsum ,sysdate-1)<=sysdate)) "
    ...
    Public Function GetRequestList(ByVal DevID As Integer) As DataTable
        ...
        cmd.CommandText = "select WEBREQEST.*,npip,nppassword,ipport,transport,sysdate ServerDate from bdevices join WEBREQEST on bdevices.id_bd=WEBREQEST.id_bd where WEBREQEST.Processed=0 and ( nplock is null or nplock <sysdate ) and npquery=1 " &
        " and ((WEBREQEST.chour=1 ) or (WEBREQEST.ccurr=1 ) or (WEBREQEST.c24=1 )  or (WEBREQEST.csum=1 )) " &
        " and bdevices.id_bd=" & DevID.ToString() + " "
    ...
    Public Function GetConfigStructFromId_BD(ByVal id_bd As Int32) As ConfigStruct
        ...
        Dim HasRows As Boolean
        cmd.Connection = connection
        cmd.CommandText = "select bdevices.npip, bdevices.nppassword, bdevices.npquery,bdevices.hiderow,  devices.cdevname, bdevices.transport, bdevices.ipport" +
    ...
    Public Function readtarch() As String
        ...
        End If
    End Function
    Public Function readarch(ByVal ArchType As Int32, ByVal ArchYear As Int32,
   ByVal ArchMonth As Int32, ByVal ArchDay As Int32, ByVal ArchHour As Int32) As String
    ...
    Public Sub SetTimeToPlanCall(ByVal id_bd As String, ByVal FieldName As String, ByVal time As DateTime)
        command.CommandText = "update PlanCall set " + FieldName +
        "=" + "to_date('" + time.Year.ToString() + "-" +
    ...
    Public Sub SetNCALLToPlanCall(ByVal id_bd As String, ByVal NCALL As Integer)
        command.CommandText = "update PlanCall set NCALL=" + NCALL.ToString +
    ...
    Private Sub TVD_TransportStatus(ByVal Action As UnitransportAction, ByVal MSG As String) Handles TVD.TransportStatus
        Select Case Action
            Case ELFDBMain.UnitransportAction.Connected

            Case ELFDBMain.UnitransportAction.Connecting

            Case ELFDBMain.UnitransportAction.Destroy

            Case ELFDBMain.UnitransportAction.Disconnected
            ...
            Case ELFDBMain.UnitransportAction.SettingUp

            Case ELFDBMain.UnitransportAction.Wait

            Case ELFDBMain.UnitransportAction.LowLevelStop
    ...
    Public Function QueryExec(ByVal s As String) As Boolean
        ...
        Try
            ...
            t = Now
            cmd.ExecuteNonQuery()
            Debug.Print(s + " dt:" + DateDiff(DateInterval.Second, t, Now).ToString)
        Catch ex As Exception
            Debug.Print(s + " err:")
            Debug.Print(ex.Message)
            Try
