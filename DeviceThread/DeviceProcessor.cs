                ...
                //if (chour || ccurr || c24 || csum)
                {
                    ...
                    if (DeviceOK)
                    {
                        ...
                        // try to load missing hour archives
                        {
                            ...
                            DateTime tempdate;
                            DataTable missing;
                            missing = TvMain.QuerySelect("select ARCHDATE from missingarch where id_bd=" + id_bdc.ToString() + " and ARCHDATE>SYSDATE-15 and ARCHDATE<SYSDATE-1 and ARCHDATE<" + TvMain.OracleDate(ddd) +"  and DEVNAME like '%Час%' "); // and devname not like '%Нули%'");
                        ...
                        // try to load missing day archives
                        {
                            ...
                            DateTime tempdate;
                            DataTable missing;
                            missing = TvMain.QuerySelect("select ARCHDATE from missingarch where id_bd=" + id_bdc.ToString() + " and ARCHDATE>SYSDATE-20 and ARCHDATE<" + TvMain.OracleDate(ddd) + " and DEVNAME like '%Суточ%'  "); //and devname not like '%Нули%'");
        ...
        private void AnalizeDevice(DataRow dr)
        {
            ...
            if (aCFG.Rows.Count > 0)
            {
                ...
                if (int.Parse(aCFG.Rows[0]["ANALIZENODE"].ToString()) != 0)
                {
                    ...
                    {
                        VIPAnalizer.NodeAnalizer na = new VIPAnalizer.NodeAnalizer();
                        na.AnalizeNode(TvMain, id_bdc, 5,false);
