...
Public Class frmGoalPower
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        ' Set chart title
        Chart1.Titles.Clear()
        Chart1.Titles.Add("Сравнение натуральных показателей за " & v2016 & " год " & vbCrLf & "(с указанием порогов снижения)" & vbCrLf & " с годовым показателем предыдущего года, тыс.кВт * ч")
        ...
        Chart1.Titles.Add("Потребление по месяцам за " & v2016 & " год, тыс. кВт*ч")
        ...
        dt = TvMain.QuerySelect("select  sum(power) POWER,SENDER_NAME  from estat join esender on senderid=sender_id where   theyear= " & v2015 & " group by SENDER_NAME order by SENDER_NAME")


        For i = 0 To dt.Rows.Count - 1
            Chart1.Series(v2015).Points.AddXY(dt.Rows(i)("SENDER_NAME"), dt.Rows(i)("POWER"))
            Chart1.Series("Экономия -" & vPower10 & "%").Points.AddXY(dt.Rows(i)("SENDER_NAME"), dt.Rows(i)("POWER") * v90)
            Chart1.Series("Экономия -" & vPower23 & "%").Points.AddXY(dt.Rows(i)("SENDER_NAME"), dt.Rows(i)("POWER") * v77)
        Next


        dt = TvMain.QuerySelect("select  sum(power) POWER,SENDER_NAME  from estat join esender on senderid=sender_id where   theyear= " & v2016 & " group by SENDER_NAME   order by SENDER_NAME")
        ...
        dt = TvMain.QuerySelect("select  sum(power) POWER,SENDER_NAME,themonth  from estat join esender on senderid=sender_id where theyear= " & v2016 & " group by SENDER_NAME,themonth   order by SENDER_NAME,themonth")
